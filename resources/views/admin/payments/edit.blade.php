...
<div class="container mx-auto mb-6">
    <div class="mb-2">
        <x-breadcrumb :links="[
            ...
            'Edit' => route('admin.payments.edit', $payment->id),
        ]" />
    </div>

    <div class="mb-4">
        <h1 class="text-2xl text-white font-semibold mb-4">Edit Payment</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-md rounded-lg p-6">
        <form action="{{ route('admin.payments.update', $payment->id) }}" method="POST" class="w-full">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <select id="user_id" name="user_id" required
                    class="shadow-xs bg-gray-50 border @error('user_id') border-red-500 @enderror text-gray-900 text-sm rounded-md 
            ...
            <div class="mb-5">
                ...
                <select id="tuition_fee_id" name="tuition_fee_id" required
                    ...
                    @foreach ($tuitionFees as $fee)
                        <option value="{{ $fee->id }}" 
                            {{ old('tuition_fee_id', $payment->tuition_fee_id) == $fee->id ? 'selected' : '' }}>
            ...
            <div class="mb-5">
                ...
                <input type="month" id="month" name="month" 
                    value="{{ old('month', \Carbon\Carbon::parse($payment->month)->format('Y-m')) }}"
